...
Namespace Chart3D_Lesson3
    ...
    Public Class IrisesViewModel
        ...
        Public Sub New()
            Irises = GetIrises("/Data/IrisDataSet.xml")
        End Sub
    End Class

    Friend Module DataLoader

        Public Function GetIrises(ByVal filepath As String) As ObservableCollection(Of IrisData)
            Dim irisDataSet As ObservableCollection(Of IrisData) = New ObservableCollection(Of IrisData)()
            Dim uri As Uri = New Uri(filepath, UriKind.RelativeOrAbsolute)
            Dim info As StreamResourceInfo = Application.GetResourceStream(uri)
            Dim document As XDocument = XDocument.Load(info.Stream)
            If document Is Nothing Then Return irisDataSet
